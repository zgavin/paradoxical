RSpec.describe "parse smoke", :parse_smoke do
  ...
  if slug.nil? || slug.empty?
    ...
    it "parses every #{parseable_exts.join("/")} file under #{slug} (root: #{game.root})" do
      ...
      # Submit all files first (non-blocking — each parse_file just
      # queues a job to the parser pool's worker Ractors), then await
      # each future in order. While we await, the pool runs N parses
      # concurrently across cores.
      submissions = files.map do |full_path|
        ...
        begin
          future = game.parse_file(arg, encoding: forced_encoding)
          { display: display, on_allowlist: on_allowlist, future: future }
        rescue Paradoxical::Parser::ParseError, EncodingError, ArgumentError => e
          { display: display, on_allowlist: on_allowlist, sync_error: e }
        end
      end

      submissions.each do |sub|
        display = sub[:display]
        on_allowlist = sub[:on_allowlist]

        last_error = sub[:sync_error]
        parsed = false
        if last_error.nil? && sub[:future] then
          begin
            sub[:future].value
            parsed = true
          rescue Paradoxical::Parser::ParseError, EncodingError, ArgumentError => e
