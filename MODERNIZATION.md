...
- **`Primitives::String`** — gains `binary_encoding:` recording one of `LOOKUP_08` / `LOOKUP_08A` / `LOOKUP_16` / `LOOKUP_16A` / `LOOKUP_24` for lookup-resolved strings. Load-bearing for round-trip because byte width alone can't disambiguate `LOOKUP_08` vs `LOOKUP_08A` (both 1 byte) or `LOOKUP_16` vs `LOOKUP_16A` (both 2). Stays nil for `QUOTED`/`UNQUOTED` strings (`quoted` is 1:1 with the wire opcode there) and for token-resolved strings (`token_index` already implies "this was a 2-byte raw token"). 10e and 10f's existing `token_index` / `lookup_index` / `quoted` kwargs unchanged.
...
**Tests:** 21 new specs covering U32/U64/I32/I64 stamping, F32/F64 wrap + stamping, `FIXED_*` stamping, plaintext-primitives leave `binary_encoding` nil, value-state semantics on `Primitives::Integer` / `Primitives::Float` (default nil, equality ignores, `dup` preserves), the `date_tokens:` allowlist (override extends, empty set suppresses, default still works), and all five `LOOKUP_*` variants on `Primitives::String#binary_encoding`. 2 existing F32/F64 specs reworked from "raw `::Float`" assertions to "wrapped `Primitives::Float` with `binary_encoding`." **636 / 0** full suite.
...
e#### 10i. Binary parser performance (landed)

The pure-Ruby binary parser took ~170s on a real ~172MB EU5 1337-start gamestate (40MB on disk). Profiled with stackprof (CPU) and `GC.stat`/`ObjectSpace.count_objects` deltas for allocation counting. **Do not run `memory_profiler` against a full save** — it retains a record per allocation and a parse makes ~380M of them, which will exhaust RAM and hard-lock the machine. `GC.stat` counters are the safe way to measure allocation here.

Two profile findings drove the work: ~30% of CPU was GC (mostly *marking*, which scales with live-object count), and a large slice went to `Elements::Concerns::Impersonator`'s `==`/`is_a?` overrides walking `Module#ancestors`.

Landed changes (`lib/paradoxical/binary/parser.rb` only — no shared-element changes):

- **Frozen control-marker singletons** (`OPEN_MARKER`/`CLOSE_MARKER`/`EQUALS_MARKER`) replace the `{ open: true }` etc. hashes that `read_scalar` allocated fresh on every control-token read.
- **`raw_int`** reads an unsigned LE integer as a plain Ruby `Integer` with an inlined bit-shift for the common 1–4 byte widths, skipping both the `Primitives::Integer` wrap and the `reverse_each.reduce` enumerator. Bookkeeping callers (the 2-byte type tag, `fixed`/`signed_integer` magnitudes, string length prefix, lookup indices) use it directly and never allocate a throwaway wrapper.
- **`n.class == Hash`** instead of `n.is_a?(Hash)` at the dispatch sites, sidestepping the Impersonator `is_a?` override (`Module#ancestors` walk) for the common wrapped-primitive case.
- **Cursor (`@pos`) over the unpacked byte array.** `raw_int` indexes `@bytes` directly with no slice; `read_bytes` (renamed from `shift_bytes`) slices + advances only where a byte sequence is genuinely needed (`string`/`float` `pack`, `bool`). An earlier cursor attempt that still sliced (`@bytes[@pos, length]`) showed *no* gain — confirming the win is avoiding the slice allocation, not `Array#shift` itself (MRI head-offset-optimizes `shift`, so it was never the O(n) cost it looked like).

Result: ~170s → ~116s (≈32%), ~59M fewer objects allocated per parse (381M → 323M). 84 binary specs unchanged.

**GC tuning explored and rejected as a library change.** Since GC is ~30%, less-aggressive GC was tempting on a 64GB box. Measured ceiling with `GC.disable`: 87s (≈25% more) but 20GB peak RSS that scales ~linearly with save size. Env-var tuning (`RUBY_GC_MALLOC_LIMIT`/`OLDMALLOC_LIMIT`/heap slots cranked high) only reached ~105–110s at ~8.7GB — because GC cost here is *marking the growing live AST*, not allocation-triggered collection frequency, so deferring GCs doesn't reduce total marking work; only disabling it does. And a `GC.disable`-wrapped parse merely defers the sweep of that 20GB to the next collection after re-enable, so the win is partly borrowed. Conclusion: GC policy is a *caller* decision, not a parser default — a one-shot bulk sweep on a big box can wrap the call in `GC.disable`/`GC.enable` itself; the library shouldn't impose 20GB+ peak on consumers that parse several saves per process or run on smaller boxes.

**Remaining headroom (not pursued):** the Impersonator `==`/`is_a?` override overhead (~30% of CPU after the above) is the next-largest lever, likely via `Property#key=`/`#value=` running `value.is_a?(VariableRef)` on every node and routing wrapped primitives through the ancestors-walking override. Deferred because it touches shared `Elements` code (blast radius beyond the binary parser) and deserves its own PR with the full suite as the net. At ~2min for a one-shot token-mapping sweep, current speed is adequate.

**Implication for a hypothetical Rust binary parser.** A long-standing idea was to push the binary parser into the Rust extension (as the script parser already is). These profiling results argue it *would not* dramatically improve parse time. The two dominant costs — ~30% GC and ~30% Impersonator dispatch — are both inherent to *materializing the Ruby `Elements` AST*, which a Rust parser must still produce and hand back: it allocates the same ~90M live `Document`/`List`/`Property`/`Value`/`Primitives::*` objects (so GC still marks them on every collection, and the marking cost is what dominates GC here), and any `Primitives::*` it constructs still carry the Impersonator concern, so downstream `==`/`is_a?` on those values cost the same regardless of who built them. Rust would speed up the byte-decode loop (`read_scalar` dispatch, `raw_int`, cursor advance) — but that's the *cheap* part: we proved the byte-stream bookkeeping (`Array#shift`, slicing) was never the bottleneck, and after this PR the decode loop is a minority of CPU. Net expectation: a Rust rewrite buys maybe the ~10–15% currently in pure-Ruby decode dispatch, not an order of magnitude, at the cost of FFI complexity and losing the pure-Ruby parser's debuggability. The real levers are Ruby-side: shrink the live AST (fewer/lighter wrapper objects) or cut the Impersonator dispatch — both of which help the existing parser without a rewrite.
...
- **GC control stays a caller concern, not a parser default (10i).** Binary parse is ~30% GC. On a 64GB box, disabling GC around the parse recovers ~25% wall-clock (87s vs 116s) but pushes peak RSS to 20GB and scales ~linearly with save size; env-var GC tuning only buys ~5–9% because the cost is marking the growing live AST, not collection frequency. Re-enabling GC after a disabled parse also just defers the big sweep rather than avoiding it. So the library never touches GC settings — a one-shot bulk consumer can wrap its own call in `GC.disable`/`GC.enable`, but imposing 20GB+ peak on every parse would break multi-parse processes and smaller boxes. Measure allocations with `GC.stat` deltas, never `memory_profiler`, on full saves (per-allocation retention OOMs the machine).
