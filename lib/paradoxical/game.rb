...
class Paradoxical::Game
  ...
  # Lazy parser-pool. Initialized on first parse_file call so Games
  # constructed but never used (e.g. unit tests) don't spawn Ractors.
  # Shared by all parse_file calls on this Game (and any Mods loaded
  # under it — see Mod#parser_pool, which delegates here).
  def parser_pool
    @parser_pool ||= Paradoxical::Parser::Pool.new
  end
  ...
  def parse_file relative_path, mod: nil, ignore_cache: false, encoding: nil
    mod ||= mod_for_path relative_path, mod: mod unless mod == false

    return super relative_path, ignore_cache: ignore_cache, encoding: encoding unless mod.present?

    mod.parse_file relative_path, ignore_cache: ignore_cache, encoding: encoding
  end

  # Parse multiple files in parallel via the parser-pool's worker
  # Ractors. Submits all jobs first (non-blocking), then awaits them
  # in order — so the pool runs N parses concurrently while we wait.
  # Returns an array of parsed `Document`s in input order, or a
  # single Document if exactly one file was passed (preserving the
  # original API).
  def parse_files *files, mod: nil, encoding: nil
    files = files.flatten
    return nil if files.empty?

    futures = files.map do |path|
      _mod = mod_for_path(path, mod: mod)
      parse_file path, mod: _mod, encoding: encoding
    end

    results = futures.map(&:value)
    files.length == 1 ? results.first : results
  end
